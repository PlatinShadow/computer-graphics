...
namespace glframework
{
    ...
    {
        if (globalState.keyCallback)
            globalState.keyCallback(key, action == GLFW_PRESS);
    ...
    void callbackFunctionMouseScroll(GLFWwindow* window, double xoffset, double yoffset)
    {
        ...
        globalState.cameraPos -= globalState.cameraPos * (float)yoffset * 0.05f;

        if (glm::length(globalState.cameraPos) < 1.0f)
            globalState.cameraPos = glm::normalize(globalState.cameraPos);
    }
    ...
    bool init(const char *windowName)
    {
        ...
        globalState.cameraPos = glm::vec3(0.0f, 0.0f, 16.0f);
    ...
    glm::mat4 getProjection()
    {
        ...
        return glm::perspective(glm::radians(30.0f), (float)windowSize.x / (float)windowSize.y, 0.1f, 100.0f);
    }
    ...
    void setKeyCallback(key_callback_func func)
    {
        globalState.keyCallback = func;
    }
    ...
    bool getKeyDown(char key)
    {
        return glfwGetKey(globalState.window, key) == GLFW_PRESS;
    }
}
